...
def fx_unwrap_optional_tensor(optional: Optional[keras.KerasTensor]) -> keras.KerasTensor:
    """Helper to unwrap optional tensors."""
    if optional is None:
        # Returning an empty tensor if cache is uninitialized, as required by graph compatibility
        return ops.zeros((0,), dtype='float32') 
    return optional

# --- Caching Helper Functions (Mirroring PyTorch structure) ---
...
def _update_kv_cache(
    ...
    if kv_caching_lengths is not None:
        ...
        # NOTE: Assumes split_2D_jagged is imported/available
...
def _construct_full_kv(
    ...
    # NOTE: Assumes concat_2D_jagged is imported/available
...
class STULayer(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        # Simplified output weight shape for demonstration (original was V*H*3)
    ...
    def call( # Standard Keras forward method
        ...
        # NOTE: Assumes hstu_preprocess_and_attention is available
        u, attn_output, k, v = hstu_preprocess_and_attention( 
            ...
            sort_by_length=self._sort_by_length, prefill=kv_caching_lengths is not None, kernel=HammerKernel.KERAS,
        )
        ...
        # NOTE: Assumes hstu_compute_output is available
        return hstu_compute_output( 
            ...
            concat_ux=True, training=training, kernel=HammerKernel.KERAS, recompute_y_in_backward=self._recompute_y,
        )
    ...
    def cached_forward( # Called for token-by-token generation
        ...
        # NOTE: Assumes hstu_compute_uqvk is available
        delta_u, delta_q, delta_k, delta_v = hstu_compute_uqvk( 
            ...
            num_heads=self._num_heads, attn_dim=self._attention_dim, hidden_dim=self._hidden_dim,
            uvqk_weight=self._uvqk_weight, uvqk_bias=self._uvqk_beta, kernel=HammerKernel.KERAS,
        )
        
        # Flatten delta_k/v from [L, H, D] to [L, H*D] for concatenation
        ...
        # NOTE: Assumes delta_hstu_mha is available
        delta_attn_output = delta_hstu_mha( 
            ...
            contextual_seq_len=self._contextual_seq_len, kernel=HammerKernel.KERAS,
        )
...
...         super().__init__(**kwargs)
...         # Register layers with Keras by setting them as attributes or list properties
